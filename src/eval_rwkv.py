...
import models_rwkv_x070
from models_rwkv_x070 import RWKV_shared
from samplers import get_data_sampler, sample_transformation
from tasks import get_task_sampler


def get_model_from_run(run_path, args, step=-1, only_conf=False):
...
def eval_batch(model, task_sampler, xs, xs_p=None):
    task = task_sampler()
    print(model.name)
    if torch.cuda.is_available() and model.name.split("_")[0] in ["RWKV-shared"]:
        device = "cuda"
    else:
        device = "cpu"
...
def build_evals(conf):
    n_dims = conf.vocab_size
    # print(f"n_dims: {n_dims}")
    n_points = conf.points_end
    batch_size = conf.batch_size

    task_name = conf.task_name
    data_name = conf.training_data
...
def get_run_metrics(
    run_path, args, step=-1, cache=True, skip_model_load=False, skip_baselines=False
):
    if skip_model_load:
        _, conf = get_model_from_run(run_path, only_conf=True)
        all_models = []
    else:
        model, conf = get_model_from_run(run_path, args)
        model = model.cuda().eval()
        all_models = [model]
        if not skip_baselines:
            all_models += models_rwkv_x070.get_relevant_baselines(conf.task_name)
...
def read_run_dir(run_dir):
    ...
    for task in os.listdir(run_dir):
        ...
        for run_id in os.listdir(task_dir):
            ...
            params["n_dims"] = conf.model.emb.weight.shape[0]
